...
for countryname in country:
    DF = (newdf[newdf["Country Name"] == countryname])
    for row in DF.iterrows():
        for i in range(len(row)):
            print(i," ",row[i])
        
countryDF = newdf[newdf['Country Name'] == 'India']
countryDF.to_csv('India.csv')
imputed_DF.to_csv(countryname+".csv")

#imputed_DF.to_csv("ImputedData.csv")
##Stats for missing data
#colmissing=(len(und)-und.count())
#print(colmissing)
#type(colmissing)
#print(len(und))
